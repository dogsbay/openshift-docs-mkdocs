...
# Custom Metrics Autoscaler Operator overview { #nodes-cma-autoscaling-custom }
...
![Custom metrics autoscaler workflow](../../images/564_OpenShift_Custom_Metrics_Autoscaler_0224.png)
...
## Custom CA certificates for the Custom Metrics Autoscaler { #nodes-cma-autoscaling-custom-ca_nodes-cma-autoscaling-custom }
...
If you want to use off-cluster services that require custom CA certificates, you can add the required certificates to a config map. Then, add the config map to the `KedaController` custom resource as described in [Installing the custom metrics autoscaler](nodes-cma-autoscaling-custom-install.md#nodes-cma-autoscaling-custom-install). The Operator loads those certificates on start-up and registers them as trusted by the Operator.
