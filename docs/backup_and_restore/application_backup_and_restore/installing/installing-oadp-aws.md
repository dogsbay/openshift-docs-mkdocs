...
## About Amazon Simple Storage Service, Identity and Access Management, and GovCloud { #oadp-s3-and-gov-cloud_installing-oadp-aws }
...
## Configuring Amazon Web Services { #migration-configuring-aws-s3_installing-oadp-aws }
...
## About backup and snapshot locations and their secrets { #oadp-about-backup-snapshot-locations_installing-oadp-aws }
...
### Backup locations { #backup-locations_installing-oadp-aws }
...
### Snapshot locations { #snapshot-locations_installing-oadp-aws }
...
### Secrets { #secrets_installing-oadp-aws }
...
### Creating a default Secret { #oadp-creating-default-secret_installing-oadp-aws }
...
### Creating profiles for different credentials { #oadp-aws-secrets-for-different-credentials_installing-oadp-aws }
...
### Creating an OADP SSE-C encryption key for additional data security { #oadp-ssec-encrypted-backups_installing-oadp-aws }
...
#### Downloading a file with an SSE-C encryption key for files backed up by Velero { #oadp-ssec-encrypted-backups-velero_installing-oadp-aws }
...
## Installing the Data Protection Application { #oadp-installing-dpa_installing-oadp-aws }
...
## Changing the default backup storage location in a DPA { #oadp-changing-default-bsl_installing-oadp-aws }
...
### Setting Velero CPU and memory resource allocations { #oadp-setting-resource-limits-and-requests_installing-oadp-aws }
...
### Enabling self-signed CA certificates { #oadp-self-signed-certificate_installing-oadp-aws }
...
### Using CA certificates with the velero command aliased for Velero deployment { #oadp-using-ca-certificates-with-velero-command-aliased-for-velero-deployment_installing-oadp-aws }
...
### Configuring node agents and node labels { #oadp-configuring-node-agents_installing-oadp-aws }
...
## Configuring the backup storage location with a MD5 checksum algorithm { #oadp-configuring-aws-md5sum_installing-oadp-aws }
...
## Configuring the DPA with client burst and QPS settings { #oadp-configuring-client-burst-qps_installing-oadp-aws }
...
## Configuring node agent load affinity { #oadp-configuring-node-agent-load-affinity_installing-oadp-aws }
...
## Node agent load affinity guidelines { #oadp-node-agent-load-affinity-guidelines_installing-oadp-aws }
...
## Configuring node agent load concurrency { #oadp-configuring-node-agent-load-concurrency_installing-oadp-aws }
...
## Configuring the node agent as a non-root and non-privileged user { #oadp-configuring-node-agent-non-root_installing-oadp-aws }
...
## Configuring repository maintenance { #oadp-configuring-repository-maintenance_installing-oadp-aws }
...
## Configuring Velero load affinity { #oadp-configuring-velero-load-affinity_installing-oadp-aws }
...
## Configuring a priority class for node agent and Velero pods { #oadp-configuring-priority-class_installing-oadp-aws }
...
## Overriding the imagePullPolicy setting in the DPA  { #oadp-configuring-imagepullpolicy_installing-oadp-aws }
...
## Enabling CSI in the DataProtectionApplication CR { #oadp-enabling-csi-dpa_installing-oadp-aws }
...
## Disabling the node agent in DataProtectionApplication { #oadp-about-disable-node-agent-dpa_installing-oadp-aws }
