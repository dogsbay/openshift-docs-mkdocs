...
## About the must-gather tool { #about-must-gather_gathering-cluster-data }
...
### Gathering data for specific resources { #support-must-gather-targeted-collection-gathering-data_gathering-cluster-data }
...
### Must-gather flags { #must-gather-flags_gathering-cluster-data }
...
### Gathering data about specific features { #gathering-data-specific-features_gathering-cluster-data }
...
<tbody>
<tr>
  <td><code>registry.redhat.io/container-native-virtualization/cnv-must-gather-rhel9:v4.22.6</code></td>
  <td>Data collection for OpenShift Virtualization.</td>
</tr>
<tr>
...
### Gathering network logs { #gathering-data-network-logs_gathering-cluster-data }
...
### Changing the must-gather storage limit { #must-gather-storage-limit_gathering-cluster-data }
...
## Self-service Technical Supportability Review { #about-self-service-tsr_gathering-cluster-data }
...
### How to access the TSR { #how-to-access-tsr_gathering-cluster-data }
...
## About Support Log Gather { #support-log-gather-overview_gathering-cluster-data }
...
### Installing Support Log Gather by using the CLI { #support-log-gather-install-cli_gathering-cluster-data }
...
### Configuring a Support Log Gather instance { #support-log-gather-config-cli_gathering-cluster-data }
...
1. Create a YAML file for the `MustGather` CR, such as `support-log-gather.yaml`, that contains the following configuration:
    ...
    ```yaml title="Example support-log-gather.yaml"
...
### Configurations for reducing the must-gather log size { #must-gather-operator-size-reduction-examples_gathering-cluster-data }
...
### Configuration parameters for MustGather custom resource { #support-log-gather-config-params_gathering-cluster-data }
...
<tr>
  ...
  <td>Optional: Overrides the default image by defining a specific custom image.<br><br><div class="admonition note"><p class="admonition-title">Note</p><p>Each <code>MustGather</code> CR supports only one custom image. To use multiple custom images, you must create a separate <code>MustGather</code> CR for each image.</p></div></td>
...
<tr>
  ...
  <td>Optional: Specifies the name of the service account. The default value is <code>default</code>.<br><br><div class="admonition note"><p class="admonition-title">Note</p><p>Because the <code>default</code> service account has minimal permissions, you can specify the service account that you created.</p></div></td>
  <td><code>string</code></td>
</tr>
<tr>
...
### Uninstalling Support Log Gather { #support-log-gather-uninstall-console_gathering-cluster-data }
...
2. Uninstall the Support Log Gather Operator.
    ...
    2. Click the Options menu ![](../images/kebab.png "Options menu") next to the **Support Log Gather** entry and click **Uninstall Operator**.
...
3. Optional: Remove the custom resource definitions (CRDs) that were installed by the Support Log Gather:
    ...
    3. Click the Options menu ![](../images/kebab.png "Options menu") next to each of the following CRDs, and select **Delete Custom Resource Definition**:
...
4. Optional: Remove the `must-gather-operator` namespace.
    ...
    2. Click the Options menu ![](../images/kebab.png "Options menu") next to the **must-gather-operator** and select **Delete Namespace**.
...
## About sosreport { #about-sosreport_gathering-cluster-data }
...
## Generating a sosreport archive for an OpenShift Container Platform cluster node { #support-generating-a-sosreport-archive_gathering-cluster-data }
...
## Querying bootstrap node journal logs { #querying-bootstrap-node-journal-logs_gathering-cluster-data }
...
## Querying cluster node journal logs { #querying-cluster-node-journal-logs_gathering-cluster-data }
...
## Network trace methods { #support-network-trace-methods_gathering-cluster-data }
...
## Collecting a host network trace { #support-collecting-host-network-trace_gathering-cluster-data }
...
## Collecting a network trace from an OpenShift Container Platform node or container { #support-collecting-network-trace_gathering-cluster-data }
...
## Providing diagnostic data to Red Hat Support { #support-providing-diagnostic-data-to-red-hat_gathering-cluster-data }
...
## About `toolbox` { #about-toolbox_gathering-cluster-data }
...
### Installing packages to a `toolbox` container { #installing-packages-to-a-toolbox-container_gathering-cluster-data }
...
### Starting an alternative image with `toolbox` { #starting-an-alternative-image-with-toolbox_gathering-cluster-data }
