...
## Key RHCOS features { #rhcos-key-features_architecture-rhcos }
...
### How to configure RHCOS { #rhcos-configured_architecture-rhcos }
...
### How to deploy RHCOS { #rhcos-deployed_architecture-rhcos }
...
### Ignition { #rhcos-about-ignition_architecture-rhcos }
...
## Viewing Ignition configuration files { #ignition-config-viewing_architecture-rhcos }
...
2. To see the contents of the `bootstrap.ign` file, pipe the file through the `jq` filter.
    ...
    ```terminal title="Example snippet from the bootstrap.ign file"
...
## Changing Ignition configs after installation { #digging-into-machine-config_architecture-rhcos }
