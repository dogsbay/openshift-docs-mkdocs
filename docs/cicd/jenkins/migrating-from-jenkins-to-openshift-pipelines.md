...
## Comparison of Jenkins and OpenShift Pipelines concepts { #jt-comparison-of-jenkins-and-openshift-pipelines-concepts_migrating-from-jenkins-to-openshift-pipelines }

You can review and compare the following equivalent terms used in Jenkins and OpenShift Pipelines.

### Jenkins terminology { #_jenkins_terminology }
...
### Mapping of concepts { #_mapping_of_concepts }
...
## Migrating a sample pipeline from Jenkins to OpenShift Pipelines { #jt-migrating-a-sample-pipeline-from-jenkins-to-openshift-pipelines_migrating-from-jenkins-to-openshift-pipelines }
...
### Jenkins pipeline { #_jenkins_pipeline }
...
### OpenShift Pipelines pipeline { #_pipelines_shortname_pipeline }
...
```yaml title="Example build task YAML definition file"
...
```yaml title="Example test task YAML definition file"
apiVersion: tekton.dev/v1beta1
kind: Task
metadata:
  name: myproject-test
...
```yaml title="Example deploy task YAML definition file"
apiVersion: tekton.dev/v1beta1
kind: Task
metadata:
...
```yaml title="Example: OpenShift Pipelines pipeline for building, testing, and deployment"
apiVersion: tekton.dev/v1beta1
kind: Pipeline
metadata:
...
## Migrating from Jenkins plugins to Tekton Hub tasks { #jt-migrating-from-jenkins-plugins-to-openshift-pipelines-hub-tasks_migrating-from-jenkins-to-openshift-pipelines }
...
## Extending OpenShift Pipelines capabilities using custom tasks and scripts { #jt-extending-openshift-pipelines-capabilities-using-custom-tasks-and-scripts_migrating-from-jenkins-to-openshift-pipelines }
...
## Comparison of Jenkins and OpenShift Pipelines execution models { #jt-comparison-of-jenkins-openshift-pipelines-execution-models_migrating-from-jenkins-to-openshift-pipelines }
...
## Examples of common use cases { #jt-examples-of-common-use-cases_migrating-from-jenkins-to-openshift-pipelines }
...
### Running a Maven pipeline in Jenkins and OpenShift Pipelines { #_running_a_maven_pipeline_in_jenkins_and_pipelines_shortname }
...
```yaml title="Example: Compile and build an image and deploy it to OpenShift using Maven in OpenShift Pipelines."
apiVersion: tekton.dev/v1beta1
kind: Pipeline
metadata:
  name: maven-pipeline
...
### Sharing reusable code in Jenkins and OpenShift Pipelines { #_sharing_reusable_code_in_jenkins_and_pipelines_shortname }
