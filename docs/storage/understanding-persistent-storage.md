...
## Persistent storage overview { #persistent-storage-overview_understanding-persistent-storage }
...
## Lifecycle of a volume and claim { #lifecycle-volume-claim_understanding-persistent-storage }
...
### Provision storage { #provisioning_understanding-persistent-storage }
...
### Bind claims { #binding_understanding-persistent-storage }
...
### Use pods and claimed PVs { #using-pods_understanding-persistent-storage }
...
### Storage Object in Use Protection { #pvcprotection_understanding-persistent-storage }
...
### Release a persistent volume { #releasing_understanding-persistent-storage }
...
### Reclaim policy for persistent volumes { #reclaiming_understanding-persistent-storage }
...
### Reclaiming a persistent volume manually { #reclaim-manual_understanding-persistent-storage }
...
### Changing the reclaim policy of a persistent volume { #reclaim-policy_understanding-persistent-storage }
...
## Persistent volumes { #persistent-volumes_understanding-persistent-storage }
...
### Types of PVs { #types-of-persistent-volumes_understanding-persistent-storage }
...
### Capacity { #pv-capacity_understanding-persistent-storage }
...
### Access modes { #pv-access-modes_understanding-persistent-storage }
...
<tr>
  <td>ReadOnlyMany</td>
  <td><code>ROX</code></td>
  <td>The volume can be mounted as read-only by many nodes.</td>
</tr>
<tr>
  <td>ReadWriteMany</td>
  <td><code>RWX</code></td>
  <td>The volume can be mounted as read/write by many nodes.</td>
</tr>
</tbody>
</table>
...
### Phase { #pv-phase_understanding-persistent-storage }
...
## Persistent volume claims { #storage-persistent-storage-pvc_understanding-persistent-storage }
...
## Volume Attributes Classes { #storage-persistent-storage-pvc-volumeattributesclass_understanding-persistent-storage }
...
### Volume Attributes Classes Limitations { #storage-persistent-storage-pvc-volumeattributesclass-limitations_understanding-persistent-storage }
...
### Defining Volume Attributes Classes { #storage-persistent-storage-pvc-volumeattributesclass-definingunderstanding-persistent-storage }
...
```yaml title="Example VolumeAttributesClass AWS EBS definition"
...
```yaml title="Example VolumeAttributesClass GPC PD definition"
apiVersion: storage.k8s.io/v1
kind: VolumeAttributesClass
metadata:
  name: silver
...
### Applying a Volume Attributes Class to a PVC { #storage-persistent-storage-pvc-volumeattributesclass-apply-vac_understanding-persistent-storage }
...
### Deleting Volume Attributes Classes { #storage-persistent-storage-pvc-volumeattributesclass-delete-vac_understanding-persistent-storage }
...
## Block volume support { #block-volume-support_understanding-persistent-storage }
...
### Block volume examples { #block-volume-examples_understanding-persistent-storage }
...
```yaml title="Pod specification example"
apiVersion: v1
kind: Pod
metadata:
...
### Changing fsGroup at the namespace level { #using_fsGroup_namespace_understanding-persistent-storage }
...
### Changing fsGroup at the pod level { #using_fsGroup_pod_understanding-persistent-storage }
...
## Reducing pod timeouts using seLinuxChangePolicy { #using_selinuxChangePolicy_overview_understanding-persistent-storage }
...
### RWOP and SELinux mount option { #using_selinuxChangePolicy_overview-mount-option-rwop_understanding-persistent-storage }
...
### Testing the RWO and RWX and SELinux mount option feature { #using_selinuxChangePolicy_testing-mountoption-rwo-rwx_understanding-persistent-storage }
...
### Opting out of the SELinux mount option default { #using_selinuxChangePolicy_pod-opt-out_understanding-persistent-storage }
...
#### Changing seLinuxChangePolicy at the namespace level { #using_selinuxChangePolicy_namespace_understanding-persistent-storage }
...
#### Changing seLinuxChangePolicy at the pod level { #using_selinuxChangePolicy_pod_understanding-persistent-storage }
