...
## How etcd works { #how-etcd-works_etcd-overview }
...
## Factors that affect etcd performance { #understand-etcd-performance_etcd-overview }
...
```yaml title="OpenShift Container Platform timer conditions for etcd"
...
### Effects of network latency and jitter on etcd { #etcd-perf-network-latency-jitter_etcd-overview }
...
### Effects of consensus latency on etcd { #etcd-perf-consensus-latency_etcd-overview }
...
### etcd peer round-trip time impacts on performance { #etcd-perf-rtt_etcd-overview }
...
### Effects of database size on etcd { #etcd-perf-db-size_etcd-overview }
...
### Effects of the Kubernetes API transaction rate on etcd { #etcd-perf-kube-api_etcd-overview }
