...
## Preparing AWS to use OADP for disaster recovery { #hcp-prepare-aws-oadp_hcp-disaster-recovery-oadp }
...
## Preparing bare metal to use OADP for disaster recovery { #hcp-prepare-bm-oadp_hcp-disaster-recovery-oadp }
...
## Data plane workload backup { #hcp-backup-dp-oadp_hcp-disaster-recovery-oadp }
...
## Control plane workload backup { #hcp-backup-cp-oadp_hcp-disaster-recovery-oadp }
...
### Backing up the control plane workload on AWS { #hcp-dr-oadp-backup-cp-workload-aws_hcp-disaster-recovery-oadp }
...
8. Create a YAML file that defines the `Backup` CR:
    ...
    ```yaml title="Example backup-control-plane.yaml file"
...
### Backing up the control plane workload on a bare metal { #hcp-dr-oadp-backup-cp-workload-bm_hcp-disaster-recovery-oadp }
...
## Hosted cluster restoration by using OADP { #hcp-restoring-oadp_hcp-disaster-recovery-oadp }

You can restore a hosted cluster into the same management cluster or into a new management cluster.

### Restoring a hosted cluster into the same management cluster by using OADP { #hcp-dr-oadp-restore_hcp-disaster-recovery-oadp }
...
### Restoring a hosted cluster into a new management cluster by using OADP { #hcp-dr-oadp-restore-new-mgmt_hcp-disaster-recovery-oadp }
...
## Observing the backup and restore process { #hcp-dr-oadp-observe_hcp-disaster-recovery-oadp }
...
## Using the OADP CLI to describe the Backup and Restore resources { #hcp-dr-oadp-observe-velero_hcp-disaster-recovery-oadp }
