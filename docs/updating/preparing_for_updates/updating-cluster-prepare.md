...
# Preparing to update to OpenShift Container Platform 4.22 { #updating-cluster-prepare }
...
## Kubernetes API removals { #kube-api-removals_updating-cluster-prepare }

There are no Kubernetes API removals in this release.

## Providing an administrator acknowledgment for Microsoft Azure or VMware vSphere clusters { #update-preparing-azure-vsphere-ack_updating-cluster-prepare }

If you are updating a Microsoft Azure or VMware vSphere cluster from OpenShift Container Platform 4.21 to 4.22, and you have not configured the `managedBootImages` parameter, the update is blocked with a "*This cluster is Azure or vSphere but lacks a boot image configuration.*" message. {.\_abstract}
...
## Self-service Technical Supportability Review { #about-self-service-tsr_updating-cluster-prepare }
...
### When to use the self-service TSR tool { #when-to-use-self-service-tsr_updating-cluster-prepare }
...
### How to access the TSR { #how-to-access-tsr_updating-cluster-prepare }
...
## The risk of conditional updates { #update-preparing-conditional_updating-cluster-prepare }
...
## etcd backups before cluster updates { #update-etcd-backup_updating-cluster-prepare }
...
## Using the oc adm upgrade recommend command to identify update risks { #oc-adm-upgrade-recommend_updating-cluster-prepare }
...
### Adding custom alerts to oc adm upgrade recommend command output { #oc-adm-upgrade-recommend-custom-alert_updating-cluster-prepare }
...
### Accepting risks with the oc adm upgrade recommend command { #oc-adm-upgrade-recommend-accept_updating-cluster-prepare }
...
## Preparing for Gateway API management succession by the Ingress Operator { #nw-ingress-gateway-api-manage-succession_updating-cluster-prepare }
...
- [Gateway API implementation for OpenShift Container Platform](../../networking/ingress_load_balancing/configuring_gateway_api/understand-gateway-api.md#gateway-api-implementation-specifics_understand-gateway-api)

## Best practices for cluster updates { #update-best-practices_updating-cluster-prepare }
...
### Choose versions recommended by the OpenShift Update Service { #recommended-versions_updating-cluster-prepare }
...
### Address all critical alerts on the cluster { #critical-alerts_updating-cluster-prepare }
...
### SDN support removal { #sdn-support-removal }
...
### Ensure that enough spare nodes are available { #nodes-ready_updating-cluster-prepare }
...
### Ensure that the cluster’s PodDisruptionBudget is properly configured { #pod-disruption-budget_updating-cluster-prepare }
...
## Minimizing worker node deployment time { #minimizing-worker-node-deployment-time_updating-cluster-prepare }
