...
# Updating a cluster using the CLI { #updating-cluster-cli }

You can perform minor version and patch updates on an OpenShift Container Platform cluster by using the OpenShift CLI (`oc`).

## About updating single node OpenShift Container Platform { #update-single-node-openshift_updating-cluster-cli }
...
## Prerequisites for a cluster update { #updating-cli-prereqs_updating-cluster-cli }
...
## Updating a cluster by using the CLI { #update-upgrading-cli_updating-cluster-cli }
...
## Cluster update status using oc adm upgrade status { #update-upgrading-oc-adm-upgrade-status_updating-cluster-cli }
...
## Changing the update server by using the CLI { #update-changing-update-server-cli_updating-cluster-cli }
