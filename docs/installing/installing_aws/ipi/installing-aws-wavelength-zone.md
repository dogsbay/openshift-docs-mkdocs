...
## Infrastructure prerequisites { #aws-zones-prerequisites_installing-aws-wavelength-zone }
...
- You added permissions for creating network resources that support AWS Wavelength Zones to the Identity and Access Management (IAM) user or role. For example:
    ...
    ```yaml title="Example of an additional IAM policy that attached ec2:ModifyAvailabilityZoneGroup, ec2:CreateCarrierGateway, and ec2:DeleteCarrierGateway permissions to a user or role"
    {
      "Version": "2012-10-17",
      "Statement": [
        {
...
## Opting in to an AWS Wavelength Zones { #_opting_in_to_an_aws_zone_type }
...
## Tested instance types for AWS { #installation-aws-tested-machine-types_installing-aws-wavelength-zone }
...
## Creating the installation configuration file { #installation-generate-aws-user-infra-install-config_installing-aws-wavelength-zone }
...
## Examples of installation configuration files with edge compute pools { #installation-aws-edge-compute-pools-examples_installing-aws-wavelength-zone }
...
## Cluster installation options for an AWS Wavelength Zones environment { #_cluster_installation_options_for_an_aws_wavelength_zones_environment }
...
## Install a cluster quickly in AWS Wavelength Zones { #_install_a_cluster_quickly_in_aws_wavelength_zones }
...
### Modifying an installation configuration file to use Amazon Web Services (AWS) Wavelength Zones { #_modifying_an_installation_configuration_file_to_use_aws_first_wavelength_zones }
...
1. Modify the `install-config.yaml` file by specifying Wavelength Zones names in the `platform.aws.zones` property of the edge compute pool.
    ...
    :   Specifies the list of Wavelength Zones names to use. The zones must exist in the same AWS Region specified in the `platform.aws.region` field. The following example shows a configuration for installing a cluster in the `us-west-2` AWS Region that extends edge nodes to Wavelength Zones in `Los Angeles` and `Las Vegas` locations:
...
## Installing a cluster in an existing VPC that has Wavelength Zone subnets { #creating-aws-wavelength-zone-environment-existing_installing-aws-wavelength-zone }
...
### Creating a VPC in Amazon Web Services (AWS) { #installation-creating-aws-vpc-localzone_installing-aws-wavelength-zone }
...
### CloudFormation template for the VPC { #installation-cloudformation-vpc-localzone_installing-aws-wavelength-zone }

You can use a CloudFormation template to deploy the VPC that you need for your OpenShift Container Platform cluster.

See the following template:

??? note "CloudFormation template for the VPC"
...
### Creating a VPC carrier gateway { #installation-creating-aws-vpc-carrier-gw_installing-aws-wavelength-zone }
...
### CloudFormation template for the VPC Carrier Gateway { #installation-cloudformation-vpc-carrier-gw_installing-aws-wavelength-zone }
...
### Creating subnets in Wavelength Zones { #installation-creating-aws-vpc-subnets-wz_installing-aws-wavelength-zone }
...
### CloudFormation template for the VPC subnet { #installation-cloudformation-subnet-localzone_installing-aws-wavelength-zone }
...
### Modifying an installation configuration file to use Amazon Web Services (AWS) Wavelength Zones subnets { #_modifying_an_installation_configuration_file_to_use_aws_first_wavelength_zones_subnets }
...
<br>.Example installation configuration file with Wavelength Zones subnets ` yaml     # ...     platform:       aws:         region: us-west-2         subnets:         - publicSubnetId-1         - publicSubnetId-2         - publicSubnetId-3         - privateSubnetId-1         - privateSubnetId-2         - privateSubnetId-3         - publicOrPrivateSubnetID-Wavelength-1     # ...      ` <br>`platform.aws.subnets` specifies the list of subnet IDs created in the zones: Availability and Wavelength Zones.
...
## Optional: Assign public IP addresses to edge compute nodes { #installing-with-edge-node-public_installing-aws-wavelength-zone }
...
2. Edit the machine set manifest that the installation program generates for the Wavelength Zones, so that the manifest gets deployed in public subnets. Specify `true` for the `spec.template.spec.providerSpec.value.publicIP` parameter.
    ...
    ```yaml title="Example machine set manifest configuration for installing a cluster quickly in Wavelength Zones"
    spec:
      template:
        spec:
          providerSpec:
    ...
    ```yaml title="Example machine set manifest configuration for installing a cluster in an existing VPC that has Wavelength Zones subnets"
...
## Logging in to the cluster by using the CLI { #cli-logging-in-kubeadmin_installing-aws-wavelength-zone }
...
## Logging in to the cluster by using the web console { #logging-in-by-using-the-web-console_installing-aws-wavelength-zone }
...
## Verifying nodes that were created with edge compute pool { #machine-edge-pool-review-nodes_installing-aws-wavelength-zone }
