...
## Alternatives to storing administrator-level secrets in the kube-system project { #alternatives-to-storing-admin-secrets-in-kube-system_configuring-iam-ibm-cloud }
...
Storing an administrator-level credential secret in the cluster `kube-system` project is not supported for IBM Cloud(R); therefore, you must set the `credentialsMode` parameter for the CCO to `Manual` when installing OpenShift Container Platform and manage your cloud credentials manually.
...
- [Rotating API keys for IBM Cloud(R)](../../post_installation_configuration/changing-cloud-credentials-configuration.md#refreshing-service-ids-ibm-cloud_changing-cloud-credentials-configuration)
