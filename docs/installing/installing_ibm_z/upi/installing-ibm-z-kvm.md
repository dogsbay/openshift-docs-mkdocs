---
title: Installing a cluster with RHEL KVM on IBM Z and IBM LinuxONE
---

# Installing a cluster with RHEL KVM on IBM Z and IBM LinuxONE { #installing-ibm-z-kvm }

You can install OpenShift Container Platform on IBM Z(R) or IBM(R) LinuxONE by using RHEL KVM on infrastructure that you provision, giving you full control over networking, storage, and compute resources.

!!! note

    While this document refers only to IBM Z(R), all information in it also applies to IBM(R) LinuxONE.

## Prerequisites for installing a cluster on IBM Z with RHEL { #prereqs-ibm-z-kvm-upi_installing-ibm-z-kvm }

Before you install OpenShift Container Platform on IBM Z(R) with RHEL KVM by using user-provisioned infrastructure, you must complete prerequisite tasks that prepare your KVM host, storage, and network environment.
...
## Preparing the user-provisioned infrastructure { #installation-infrastructure-user-infra_installing-ibm-z-kvm }
...
### Example load balancer configuration for user-provisioned clusters { #installation-load-balancing-user-infra-example_installing-ibm-z-kvm }
...
## Manually creating the installation configuration file { #installation-initializing-manual_installing-ibm-z-kvm }
...
### Configuring the cluster-wide proxy during installation { #installation-configure-proxy_installing-ibm-z-kvm }
...
### Configuring a three-node cluster { #installation-three-node-cluster_installing-ibm-z-kvm }
...
## Cluster Network Operator configuration { #nw-operator-cr_installing-ibm-z-kvm }
...
### Cluster Network Operator configuration object { #nw-operator-cr-cno-object_installing-ibm-z-kvm }
...
### defaultNetwork object configuration { #nw-operator-cr-defaultnetwork_installing-ibm-z-kvm }
...
### Configuration for the OVN-Kubernetes network plugin { #nw-operator-configuration-parameters-for-ovn-sdn_installing-ibm-z-kvm }
...
<a name="gatewayConfig-object_installing-ibm-z-kvm"></a>
...
<a name="gatewayconfig-ipv4-object_installing-ibm-z-kvm"></a>
...
<a name="gatewayconfig-ipv6-object_installing-ibm-z-kvm"></a>
...
<a name="nw-operator-cr-ipsec_installing-ibm-z-kvm"></a>
...
## Creating the Kubernetes manifest and Ignition config files { #installation-user-infra-generate-k8s-manifest-ignition_installing-ibm-z-kvm }
...
## Installing RHCOS and starting the OpenShift Container Platform bootstrap process { #installation-ibm-z-kvm-user-infra-installing-rhcos_installing-ibm-z-kvm }
...
To add further security to your system, you can optionally install RHCOS using IBM(R) Secure Execution before proceeding to the fast-track installation.

### Configuring encryption for nodes in an IBM Z or IBM LinuxONE environment { #configuring-encryption-kvm-ibm-z-linuxone-environment_installing-ibm-z-kvm }

When installing OpenShift Container Platform on IBM Z(R) or IBM(R) LinuxONE with RHEL KVM, you can optionally encrypt the boot volumes of your control plane and compute nodes by using one of the following methods.

- IBM(R) Secure Execution
- Linux Unified Key Setup (LUKS) encryption via IBM(R) Crypto Express (CEX)
- Network Bound Disk Encryption (NBDE)

#### Installing RHCOS using IBM Secure Execution { #installing-rhcos-using-ibm-secure-execution_installing-ibm-z-kvm }

You can install RHCOS using IBM(R) Secure Execution to run nodes as protected guests, isolating workloads from the host system. Before you begin, you must prepare the underlying infrastructure and verify hardware and software prerequisites.
...
1. Prepare your RHEL KVM host to support IBM(R) Secure Execution.
    ...
    - By default, KVM hosts do not support guests in IBM(R) Secure Execution mode. To support guests in IBM(R) Secure Execution mode, KVM hosts must boot in LPAR mode with the kernel parameter specification `prot_virt=1`. To enable `prot_virt=1` on RHEL 8, follow these steps:
        ...
        3. Run the `zipl` command and reboot your system.
            ...
            KVM hosts that successfully start with support for IBM(R) Secure Execution for Linux issue the following kernel message:
...
- [Linux as an IBM(R) Secure Execution host or guest](https://www.ibm.com/docs/en/linux-on-systems?topic=ibm-z-secure-execution)
- [Setting up IBM(R) Secure Execution on IBM Z](https://access.redhat.com/documentation/en-us/red_hat_enterprise_linux/9/html/configuring_and_managing_virtualization/securing-virtual-machines-in-rhel_configuring-and-managing-virtualization#setting-up-secure-execution-on-ibm-z_securing-virtual-machines-in-rhel)

#### LUKS encryption via CEX in an IBM Z or IBM LinuxONE environment { #configuring-luks-encryption-via-cex-ibm-z-linuxone-environment_installing-ibm-z-kvm }
...
#### Configuring NBDE with static IP in an IBM Z or IBM LinuxONE environment { #configuring-nbde-static-ip-ibm-z-linuxone-environment_installing-ibm-z-kvm }
...
### Fast-track installation by using a prepackaged QCOW2 disk image { #installation-user-infra-machines-iso-ibm-z_kvm_installing-ibm-z-kvm }
...
4. Create the new KVM guest nodes using the Ignition file and the new disk image.
    ...
    The `--launchSecurity type="s390-pv"` parameter is required only if IBM(R) Secure Execution is enabled. If IBM(R) Secure Execution is enabled, also replace `serial=ignition` with `serial=ignition_crypted` in the `--disk` parameter.

### Full installation on a new QCOW2 disk image { #installation-user-infra-machines-iso-ibm-z-kvm-full_installing-ibm-z-kvm }
...
### Networking options for ISO installations { #_networking_options_for_iso_installations }
...
#### Configuring DHCP or static IP addresses { #configuring-dhcp-or-static-ip-addresses_installing-ibm-z-kvm }
...
#### Configuring an IP address without a static hostname { #configuring-ip-address-without-static-hostname_installing-ibm-z-kvm }
...
#### Specifying multiple network interfaces and DNS servers { #specifying-multiple-network-interfaces_installing-ibm-z-kvm }
...
#### Configuring default gateway and route { #configuring-default-gateway-route_installing-ibm-z-kvm }
...
#### Configuring VLANs on individual interfaces { #configuring-vlans-individual-interfaces_installing-ibm-z-kvm }
...
## Waiting for the bootstrap process to complete { #installation-installing-bare-metal_installing-ibm-z-kvm }
...
## Logging in to the cluster by using the CLI { #cli-logging-in-kubeadmin_installing-ibm-z-kvm }
...
## Approving the certificate signing requests for your machines { #installation-approve-csrs_installing-ibm-z-kvm }
...
## Initial Operator configuration { #installation-operators-config_installing-ibm-z-kvm }
...
### Image registry storage configuration { #installation-registry-storage-config_installing-ibm-z-kvm }
...
#### Configuring storage for the image registry in non-production clusters { #installation-registry-storage-non-production_installing-ibm-z-kvm }
...
## Completing installation on user-provisioned infrastructure { #installation-complete-user-infra_installing-ibm-z-kvm }
...
## Telemetry access for OpenShift Container Platform { #cluster-telemetry_installing-ibm-z-kvm }
