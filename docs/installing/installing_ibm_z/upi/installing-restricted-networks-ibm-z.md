---
title: Installing a cluster with z/VM on IBM Z and IBM LinuxONE in a disconnected environment
---

# Installing a cluster with z/VM on IBM Z and IBM LinuxONE in a disconnected environment { #installing-restricted-networks-ibm-z }

You can install OpenShift Container Platform on IBM Z(R) or IBM(R) LinuxONE by using z/VM on infrastructure that you provision in a disconnected environment, using an internal mirror of the installation release content.

!!! note

    While this document refers to only IBM Z(R), all information in it also applies to IBM(R) LinuxONE.

## Prerequisites for installing a cluster on IBM Z in a disconnected environment { #prereqs-ibm-z-upi-disconnected_installing-restricted-networks-ibm-z }

Before you install OpenShift Container Platform on IBM Z(R) using user-provisioned infrastructure in a disconnected environment, you must complete prerequisite tasks that prepare your mirrored registry, storage, and network environment.

- You have completed the tasks in preparing to install a cluster on IBM Z(R) using user-provisioned infrastructure.
...
## About installations in restricted networks { #installation-about-restricted-networks_installing-restricted-networks-ibm-z }
...
### Additional limits { #installation-restricted-network-limits_installing-restricted-networks-ibm-z }
...
## Preparing the user-provisioned infrastructure { #installation-infrastructure-user-infra_installing-restricted-networks-ibm-z }
...
### Example load balancer configuration for user-provisioned clusters { #installation-load-balancing-user-infra-example_installing-restricted-networks-ibm-z }
...
## Manually creating the installation configuration file { #installation-initializing-manual_installing-restricted-networks-ibm-z }
...
- [Installation configuration parameters for IBM Z(R)](../installation-config-parameters-ibm-z.md#installation-config-parameters-ibm-z)

### Sample install-config.yaml file for IBM Z { #_sample_install-configyaml_file_for_ibm_z_title }
...
### Configuring the cluster-wide proxy during installation { #installation-configure-proxy_installing-restricted-networks-ibm-z }
...
### Configuring a three-node cluster { #installation-three-node-cluster_installing-restricted-networks-ibm-z }
...
## Cluster Network Operator configuration { #nw-operator-cr_installing-restricted-networks-ibm-z }
...
### Cluster Network Operator configuration object { #nw-operator-cr-cno-object_installing-restricted-networks-ibm-z }
...
### defaultNetwork object configuration { #nw-operator-cr-defaultnetwork_installing-restricted-networks-ibm-z }
...
### Configuration for the OVN-Kubernetes network plugin { #nw-operator-configuration-parameters-for-ovn-sdn_installing-restricted-networks-ibm-z }
...
<a name="gatewayConfig-object_installing-restricted-networks-ibm-z"></a>
...
<a name="gatewayconfig-ipv4-object_installing-restricted-networks-ibm-z"></a>
...
<a name="gatewayconfig-ipv6-object_installing-restricted-networks-ibm-z"></a>
...
<a name="nw-operator-cr-ipsec_installing-restricted-networks-ibm-z"></a>
...
## Creating the Kubernetes manifest and Ignition config files { #installation-user-infra-generate-k8s-manifest-ignition_installing-restricted-networks-ibm-z }
...
## Configuring boot volume encryption in an IBM Z or IBM LinuxONE environment { #configuring-boot-volume-encryption-ibm-z-linuxone-environment_installing-restricted-networks-ibm-z }

You can optionally encrypt the boot volumes of your OpenShift Container Platform control plane and compute nodes on IBM Z(R) or IBM(R) LinuxONE by using LUKS encryption via IBM(R) Crypto Express (CEX) or Network Bound Disk Encryption (NBDE).

- Linux Unified Key Setup (LUKS) encryption via IBM(R) Crypto Express (CEX)
- Network Bound Disk Encryption (NBDE)

### LUKS encryption via CEX in an IBM Z or IBM LinuxONE environment { #configuring-luks-encryption-via-cex-ibm-z-linuxone-environment_installing-restricted-networks-ibm-z }

Enabling hardware-based Linux Unified Key Setup (LUKS) encryption via IBM(R) Crypto Express (CEX) in an IBM Z(R) or IBM(R) LinuxONE environment requires additional steps.
...
### Configuring NBDE with static IP in an IBM Z or IBM LinuxONE environment { #configuring-nbde-static-ip-ibm-z-linuxone-environment_installing-restricted-networks-ibm-z }

Enabling NBDE disk encryption in an IBM Z(R) or IBM(R) LinuxONE environment requires additional steps.
...
## Installing RHCOS and starting the OpenShift Container Platform bootstrap process { #installation-user-infra-machines-iso-ibm-z_installing-restricted-networks-ibm-z }

To install OpenShift Container Platform on IBM Z(R) infrastructure that you provision, you must install Red Hat Enterprise Linux CoreOS (RHCOS) on z/VM guest virtual machines.
...
- If you want to enable secure boot, you have obtained the appropriate Red Hat Product Signing Key and read [Secure boot on IBM Z and IBM LinuxONE](https://www.ibm.com/docs/en/linux-on-systems?topic=security-secure-boot-linux-onibm-z-linuxone) in IBM(R) documentation.
...
3. Create parameter files. The following parameters are specific for a particular virtual machine:
    ...
    - For installations on DASD-type disks, complete the following tasks:
        ...
        3. Leave all other parameters unchanged.
            ...
            `coreos.inst.secure_ipl`
            :   Specifies the `coreos.inst.secure_ipl` artifact. Optional: To enable secure boot, add `coreos.inst.secure_ipl`. Write all options in the parameter file as a single line and make sure you have no newline characters.
...
9. IPL the bootstrap machine from the reader:
    ...
    See [IPL](https://www.ibm.com/docs/en/zvm/latest?topic=commands-ipl) (IBM(R) Documentation).

10. Repeat this procedure for the other machines in the cluster.

### Networking and bonding options for ISO installations { #installation-user-infra-machines-routing-bonding_installing-restricted-networks-ibm-z }
...
#### Configuring DHCP or static IP addresses { #configuring-dhcp-or-static-ip-addresses_installing-restricted-networks-ibm-z }
...
#### Configuring an IP address without a static hostname { #configuring-ip-address-without-static-hostname_installing-restricted-networks-ibm-z }
...
#### Specifying multiple network interfaces and DNS servers { #specifying-multiple-network-interfaces_installing-restricted-networks-ibm-z }
...
#### Configuring default gateway and route { #configuring-default-gateway-route_installing-restricted-networks-ibm-z }
...
#### Configuring VLANs on individual interfaces { #configuring-vlans-individual-interfaces_installing-restricted-networks-ibm-z }
...
#### Bonding multiple network interfaces to a single interface { #bonding-multiple-network-interfaces-to-single-interface_installing-restricted-networks-ibm-z }
...
#### Using network teaming { #_using_network_teaming }
...
## Waiting for the bootstrap process to complete { #installation-installing-bare-metal_installing-restricted-networks-ibm-z }
...
## Logging in to the cluster by using the CLI { #cli-logging-in-kubeadmin_installing-restricted-networks-ibm-z }
...
## Approving the certificate signing requests for your machines { #installation-approve-csrs_installing-restricted-networks-ibm-z }
...
## Initial Operator configuration { #installation-operators-config_installing-restricted-networks-ibm-z }
...
### Disabling the default software catalog sources { #olm-restricted-networks-operatorhub_installing-restricted-networks-ibm-z }
...
### Image registry storage configuration { #installation-registry-storage-config_installing-restricted-networks-ibm-z }
...
#### Configuring registry storage for IBM Z { #_configuring_registry_storage_for_ibm_z_title }
...
#### Configuring storage for the image registry in non-production clusters { #installation-registry-storage-non-production_installing-restricted-networks-ibm-z }
...
## Completing installation on user-provisioned infrastructure { #installation-complete-user-infra_installing-restricted-networks-ibm-z }
