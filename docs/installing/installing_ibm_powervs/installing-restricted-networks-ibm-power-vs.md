...
To install a OpenShift Container Platform cluster on IBM Power(R) Virtual Server in a disconnected environment, mirror the required release content and deploy into an existing Virtual Private Cloud (VPC) on IBM Cloud(R).

## Prerequisites { #prereqs-ibm-power-vs-disconnected_installing-restricted-networks-ibm-power-vs }

Before you install a OpenShift Container Platform cluster on IBM Power(R) Virtual Server in a restricted network, complete the prerequisite tasks to mirror images, configure an IBM Cloud(R) account, and prepare an existing Virtual Private Cloud (VPC).
...
## About installations in restricted networks { #installation-about-restricted-networks_installing-restricted-networks-ibm-power-vs }
...
### Additional limits { #installation-restricted-network-limits_installing-restricted-networks-ibm-power-vs }
...
## About using a custom Virtual Private Cloud (VPC) { #installation-custom-ibm-powervs-vpc_installing-restricted-networks-ibm-power-vs }

In OpenShift Container Platform 4.22, you can deploy a cluster into the subnets of an existing IBM(R) Virtual Private Cloud (VPC).

### Requirements for using your Virtual Private Cloud (VPC) { #_requirements_for_using_your_virtual_private_cloud_vpc }
...
### Virtual Private Cloud (VPC) validation { #installation-custom-ibm-power-vs-validation_installing-restricted-networks-ibm-power-vs }
...
### Isolation between clusters { #installation-custom-ibm-power-vs-isolation_installing-restricted-networks-ibm-power-vs }
...
## Generating a key pair for cluster node SSH access { #ssh-agent-using_installing-restricted-networks-ibm-power-vs }
...
## Exporting the API key { #installation-ibm-cloud-export-variables_installing-restricted-networks-ibm-power-vs }
...
2. Edit the `install-config.yaml` file to give the additional information that is required for an installation in a restricted network.
    ...
    3. Define the network for the VPC to install the cluster in under the parent `platform.powervs` field:
        ...
        For `platform.powervs.vpcName`, specify the name for the existing IBM Cloud(R) VPC.
...
### Minimum resource requirements for cluster installation { #installation-minimum-resource-requirements_installing-restricted-networks-ibm-power-vs }
...
### Sample customized install-config.yaml file for IBM Power Virtual Server { #installation-ibm-power-vs-config-yaml_installing-restricted-networks-ibm-power-vs }
...
### Configuring the cluster-wide proxy during installation { #installation-configure-proxy_installing-restricted-networks-ibm-power-vs }
...
## Manually creating IAM { #manually-create-iam-ibm-cloud_installing-restricted-networks-ibm-power-vs }
...
## Installing the OpenShift CLI on Linux { #cli-installing-cli-linux_installing-restricted-networks-ibm-power-vs }
...
## Installing the OpenShift CLI on Windows { #cli-installing-cli-windows_installing-restricted-networks-ibm-power-vs }
...
## Installing the OpenShift CLI on macOS { #cli-installing-cli-macos_installing-restricted-networks-ibm-power-vs }
...
## Logging in to the cluster by using the CLI { #cli-logging-in-kubeadmin_installing-restricted-networks-ibm-power-vs }
...
## Disabling the default software catalog sources { #olm-restricted-networks-operatorhub_installing-restricted-networks-ibm-power-vs }
