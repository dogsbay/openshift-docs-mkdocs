...
## Understanding bundle endpoint profiles { #zero-trust-manager-configure-endpoints_zero-trust-manager-spire-federation }
...
## Federation configuration examples { #zero-trust-manager-config-example_zero-trust-manager-spire-federation }
...
2. Repeat steps 5-7 on each cluster for every remote cluster it should federate with. For bidirectional federation, each cluster needs a `ClusterFederatedTrustDomain` resource for every other cluster.

3. Update the `SpireServer` resource on each cluster to add the `federatesWith` configuration:
...
4. Apply the updated configuration by running the following command:
...
## Using SPIRE federation with Automatic Certificate Management Environment protocol { #zero-trust-manager-automatic-management_zero-trust-manager-spire-federation }
...
## Using SPIRE federation with manual certificate management { #zero-trust-manager-manual-management_zero-trust-manager-spire-federation }
...
9. Monitor the certificate issuance by running the following command:
    ...
    ```terminal title="Example output when ready"
...
## Federation configuration field reference { #zero-trust-manager-federation-field-reference_zero-trust-manager-spire-federation }
