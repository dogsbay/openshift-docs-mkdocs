...
## Understanding service serving certificates { #understanding-service-serving_service-serving-certificate }
...
## Add a service certificate { #add-service-certificate_service-serving-certificate }
...
## Add the service CA bundle to a config map { #add-service-certificate-configmap_service-serving-certificate }
...
3. Mount the config map as a volume to each container that exists in a pod by configuring your `Deployment` object.
    ...
    ```yaml title="Example Deployment object that defines the volume for the mounted config map"
    apiVersion: apps/v1
    kind: Deployment
    metadata:
...
## Add the service CA bundle to an API service { #add-service-certificate-apiservice_service-serving-certificate }
...
## Add the service CA bundle to a mutating webhook configuration { #add-service-certificate-mutating-webhook_service-serving-certificate }
...
## Add the service CA bundle to a validating webhook configuration { #add-service-certificate-validating-webhook_service-serving-certificate }
...
## Manually rotate the generated service certificate { #rotate-service-serving_service-serving-certificate }
...
## Manually rotate the service CA certificate { #manually-rotate-service-ca_service-serving-certificate }
