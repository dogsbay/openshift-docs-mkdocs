...
## About ServiceMeshControlPlane { #ossm-about-smcp_ossm-create-smcp }
...
### Deploying the Service Mesh control plane from the web console { #ossm-control-plane-deploy-operatorhub_ossm-create-smcp }
...
### Deploying the Service Mesh control plane using the CLI { #ossm-control-plane-deploy-cli_ossm-create-smcp }
...
2. Create a `ServiceMeshControlPlane` file named `istio-installation.yaml` using the following example. The version of the Service Mesh control plane determines the features available regardless of the version of the Operator.
    ...
    ```yaml title="Example version 2.6 istio-installation.yaml"
...
### Validating your SMCP installation with the CLI { #ossm-validate-control-plane-cli_ossm-create-smcp }
...
### Configuring all control plane components to run on infrastructure nodes using the web console { #ossm-config-control-plane-infrastructure-node-console_ossm-create-smcp }
...
### Configuring individual control plane components to run on infrastructure nodes using the web console { #ossm-config-individual-control-plane-infrastructure-node-console_ossm-create-smcp }
...
### Configuring all control plane components to run on infrastructure nodes using the CLI { #ossm-config-control-plane-infrastructure-node-cli_ossm-create-smcp }
...
### Configuring individual control plane components to run on infrastructure nodes using the CLI { #ossm-config-individual-control-plane-infrastructure-node-cli_ossm-create-smcp }
...
### Verifying the Service Mesh control plane is running on infrastructure nodes { #ossm-confirm-smcp-infrastructure-node_ossm-create-smcp }
...
## About control plane and cluster-wide deployments { #ossm-about-control-plane-and-cluster-wide-deployment_ossm-create-smcp }
...
### Configuring the control plane for cluster-wide deployment with the web console { #ossm-deploy-cluster-wide-control-plane-console_ossm-create-smcp }
...
### Configuring the control plane for cluster-wide deployment with the CLI { #ossm-deploy-cluster-wide-control-plane-cli_ossm-create-smcp }
...
### Customizing the member roll for a cluster-wide mesh { #ossm-customize-smrr-cluster-wide_ossm-create-smcp }
...
## Validating your SMCP installation with Kiali { #ossm-validate-control-plane-kiali_ossm-create-smcp }
...
5. Kiali has four dashboards specifically for the namespace where the Service Mesh control plane is installed.  To view these dashboards, click the Options menu ![](../../images/kebab.png "Options menu") on the tile for the control plane namespace, for example, `istio-system`, and select one of the following options:
...
Red Hat OpenShift Service Mesh supports multiple independent control planes within the cluster. You can create reusable configurations with `ServiceMeshControlPlane` profiles. For more information, see [Creating control plane profiles](ossm-profiles-users.md#ossm-control-plane-profiles_ossm-profiles-users).

## Next steps { #_next_steps }

- Add a project to the Service Mesh so that applications can be made available. For more information, see [Adding services to a service mesh](ossm-create-mesh.md#ossm-create-mesh).
