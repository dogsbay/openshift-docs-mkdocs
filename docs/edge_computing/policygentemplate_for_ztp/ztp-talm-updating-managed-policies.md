...
# Updating managed clusters in a disconnected environment with PolicyGenTemplate resources and TALM { #ztp-topology-aware-lifecycle-manager }
...
## Setting up the disconnected environment { #talo-platform-prepare-for-update-env-setup_ztp-talm }
...
## Performing a platform update with PolicyGenTemplate CRs { #talo-platform-update-PolicyGenTemplate_ztp-talm }
...
1. Create a `PolicyGenTemplate` CR for the platform update:
    ...
    1. Save the following `PolicyGenTemplate` CR in the `du-upgrade.yaml` file: The following example shows the `PolicyGenTemplate` CR for platform update:
...
## Performing an Operator update with PolicyGenTemplate CRs { #talo-operator-update-PolicyGenTemplate_ztp-talm }
...
## Troubleshooting missed Operator updates with PolicyGenTemplate CRs { #cnf-topology-aware-lifecycle-manager-operator-troubleshooting-PolicyGenTemplate_ztp-talm }
...
## Performing a platform and an Operator update together { #talo-operator-and-platform-update_ztp-talm }
...
## Removing Performance Addon Operator subscriptions from deployed clusters with PolicyGenTemplate CRs { #talm-pao-update-PolicyGenTemplate_ztp-talm }
...
1. Change the `complianceType` to `mustnothave` for the Performance Addon Operator namespace, Operator group, and subscription in the `truecommon-ranGen.yaml` file.
...
5. Delete the Performance Addon Operator namespace, Operator group and subscription CRs from `spec.sourceFiles` in the `truecommon-ranGen.yaml` file.
...
## Precaching user-specified images with TALM on single-node OpenShift clusters { #talm-prechache-user-specified-images-concept_ztp-talm }
...
### Creating the custom resources for precaching { #talm-prechache-user-specified-images-preparing-crs_ztp-talm }
...
## About the auto-created ClusterGroupUpgrade CR for GitOps ZTP { #talo-precache-autocreated-cgu-for-ztp_ztp-talm }
