...
## Create a network policy using the CLI { #nw-networkpolicy-create-cli_creating-network-policy }
...
## Create a default deny all network policy { #nw-networkpolicy-deny-all-multi-network-policy_creating-network-policy }
...
## Create a network policy to allow traffic from external clients { #nw-networkpolicy-allow-external-clients_creating-network-policy }
...
2. Apply the policy by entering the following command. Successful output lists the name of the policy object and the `created` status.
    ...
    This policy allows traffic from all resources, including external traffic as illustrated in the following diagram: ![Allow traffic from external clients](../../../images/292_OpenShift_Configuring_multi-network_policy_1122.png)
...
## Create a network policy allowing traffic to an application from a namespace { #nw-networkpolicy-allow-traffic-from-a-namespace_creating-network-policy }
